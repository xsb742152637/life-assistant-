package com.xie.lifeassistant.util.io;
...
import com.xie.lifeassistant.util.config.IoConfig;
import com.xie.lifeassistant.util.io.file.Directorys;
...
public class WebPathUtils {
    ...
    public static Path getAttachRootPath(){
        if(rootPath == null){
            ...
            String path = ApplicationContext.getBean(IoConfig.class).getFileStorage();
    ...
    public static void cleanTempFolder(int field, int amount) {
        ...
        LocalDateTime startDate = Util.DataAndTime.getLocalDateTime(cal.getTimeInMillis());
            ...
            try (DirectoryStream<Path> directoryStream = Files.newDirectoryStream(cleanTemp)) {
                for (Path tmpPath : directoryStream) {
                    if (Files.isDirectory(tmpPath)) {
                        ...
                        try {
                            targetDate = Util.DataAndTime.getLocalDateTime(dateFormat.parse(tmpPath.getFileName().toString()).getTime());
